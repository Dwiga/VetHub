...
async function fetchLogs(id: number) {
  const res = await fetch(`/api/hotel/${id}/logs`);
...
export default function HotelBookingPage() {
  ...
  const bookingKey = ["hotel-booking", bookingId];
  const logsKey = ["hotel-logs", bookingId];

  const bookingQuery = useQuery({ queryKey: bookingKey, queryFn: () => fetchBooking(bookingId) });
  const logsQuery = useQuery({ queryKey: logsKey, queryFn: () => fetchLogs(bookingId) });

  const updateBooking = useMutation({
    mutationFn: async (data: Record<string, unknown>) => {
      const res = await fetch(`/api/hotel/${bookingId}`, {
        method: "PATCH",
    ...
    onSuccess: () => qc.invalidateQueries({ queryKey: bookingKey }),
  });

  const addLog = useMutation({
    mutationFn: async (data: Record<string, unknown>) => {
      const res = await fetch(`/api/hotel/${bookingId}/logs`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(data),
      });
    ...
    onSuccess: () => {
      ...
      qc.invalidateQueries({ queryKey: bookingKey });
    },
  });

  const deleteLog = useMutation({
    mutationFn: async (logId: number) => {
      const res = await fetch(`/api/hotel/${bookingId}/logs/${logId}`, { method: "DELETE" });
      if (!res.ok) throw new Error("Failed");
    },
  ...
  async function handleCheckOut() {
    ...
    await updateBooking.mutateAsync({ checkOut: date, status: "completed" });
  ...
  const petDisplayType = b.petSpecies ?? b.petTypeRaw ?? "";
  const ownerDisplayName = b.ownerName ?? b.guestName ?? "";
  const ownerDisplayPhone = b.ownerPhone ?? b.guestPhone ?? "";
