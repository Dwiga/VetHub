...
import { useSearchPetOwner, useSearchPet, useGetMe } from '@/lib/api-client'
...
function HotelSearchPage() {
  ...
  function handlePhoneSearch(e: React.FormEvent) {
    ...
    navigate({ to: '/hotel/search', search: { q: phone.trim() } })
  }

  function handlePetSearch(e: React.FormEvent) {
    e.preventDefault()
    setSubmittedPetName(petName.trim())
    navigate({ to: '/hotel/search', search: { q: petName.trim() } })
  }
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-3">
                {ownerPets.map((pet: any) => (
                  <Card key={pet.id} className="hover:border-primary/50 transition-colors" data-testid={`card-pet-${pet.id}`}>
