...
export const Route = createFileRoute('/vet/hotel/new/$petId')({
...
function HotelNewPage() {
  ...
  async function onSubmit(values: z.infer<typeof schema>) {
    ...
    navigate({ to: '/vet/hotel/$bookingId', params: { bookingId: String((booking as any).id) } })
  }
        ...
        <Card className="mb-5">
          <CardContent className="py-3 flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">{pet.data.species?.name} · {pet.data.ownerId}</p>
            </div>
          </CardContent>
        </Card>
      )}
