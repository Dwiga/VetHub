...
import {
  ...
  getGetHotelBookingQueryKey, getListHotelDailyLogsQueryKey,
...
export default function HotelDetailPage() {
  ...
  const bookingQuery = useGetHotelBooking(bookingId, { query: { queryKey: getGetHotelBookingQueryKey(bookingId) } });
  const logsQuery = useListHotelDailyLogs(bookingId, { query: { queryKey: getListHotelDailyLogsQueryKey(bookingId) } });
  ...
  async function handleAddLog(e: React.FormEvent) {
    ...
    await qc.invalidateQueries({ queryKey: getListHotelDailyLogsQueryKey(bookingId) });
    await qc.invalidateQueries({ queryKey: getGetHotelBookingQueryKey(bookingId) });
  ...
  return (
    <AppShell>
      <PageHeader
        ...
        subtitle={b.petSpecies ?? undefined}
        back
        backHref="/vet/hotel"
        action={
          <Button asChild size="sm" variant="ghost">
            <Link href={`/pets/${b.petId}`}><PawPrint className="h-4 w-4" /></Link>
          </Button>
        }
      />
          ...
          <CardContent className="py-4 space-y-2">
            ...
            {b.ownerName && (
              <div className="flex items-center justify-between text-sm">
                ...
                <span className="font-medium">{b.ownerName} {b.ownerPhone ? `· ${b.ownerPhone}` : ""}</span>
              </div>
            )}
            {b.notes && (
